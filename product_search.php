...
<div>
   <form action="product_search.php" method = "post" class="form-inline">
    <input id ="sear" name="product_id" class="form-control mr-sm-2" type="text" placeholder="Enter repair id" aria-label="Search" style="font-size: 12px;">
    <button class="btn btn-dark" type="submit" style="font-size: 12px">Search</button>
  </form>
</div>
...
$sql = "select * from Albedo.product where Product_Code='$Product_ID'";
...
<?php
  while($row = $result -> fetch_assoc()){
        ...
        echo '<td>'.$row['Purchased_ID'].'</td>';
        // echo "<td><a href='edit_item.php?itemCode=".$row['Product_Code']."'><i class='fas fa-pen'></i>Update</a></td>";
        // echo "<td><a href='delete_item.php?itemCode=".$row['Product_Code']."'><i class='fas fa-trash-alt'></i>Delete</a></td>";
        echo '</tr>';
    }
  }
else{
